...
struct mlua_icg_add_built_in_handler
{
    const char *name;
    ...
    int (*dependence)(struct mlua_icg_customizable_built_in_procedure_list *customizable_built_in_procedure_list);
};
 ...
 * end
 */
static int mlua_icg_add_built_in_procs_print(
...
static int mlua_icg_add_built_in_procs_type(struct multiple_error *err, \
        struct multiple_ir *icode, \
        struct multiply_resource_id_pool *res_id)
{
    int ret = 0;
...
static int mlua_icg_add_built_in_procs_tonumber(struct multiple_error *err, \
        struct multiple_ir *icode, \
        struct multiply_resource_id_pool *res_id)
{
    int ret = 0;
...
static int mlua_icg_add_built_in_procs_tostring(struct multiple_error *err, \
        struct multiple_ir *icode, \
        struct multiply_resource_id_pool *res_id)
{
    int ret = 0;
...
/* Also check mlua_icg_fcb_built_in_proc.h */

static struct mlua_icg_add_built_in_handler mlua_icg_add_built_in_handlers[] = 
{
    {"print", 5, mlua_icg_add_built_in_procs_print, NULL},
    {"type", 4, mlua_icg_add_built_in_procs_type, NULL},
    {"tonumber", 8, mlua_icg_add_built_in_procs_tonumber, NULL},
    {"tostring", 8, mlua_icg_add_built_in_procs_tostring, NULL},
};
#define MLUA_ICG_ADD_BUILT_IN_HANDLERS_COUNT (sizeof(mlua_icg_add_built_in_handlers)/sizeof(struct mlua_icg_add_built_in_handler))

int mlua_icg_add_built_in_procs(struct multiple_error *err, \
        struct multiple_ir *icode, \
        struct multiply_resource_id_pool *res_id, \
        ...
        uint32_t insert_point, uint32_t *instrument_count)
{
    ...
    *instrument_count = 0;
    ...
    while (customizable_built_in_procedure_cur != NULL)
    {
        ...
        {
            for (i = 0; i != MLUA_ICG_ADD_BUILT_IN_HANDLERS_COUNT; i++)
            {
                if ((mlua_icg_add_built_in_handlers[i].name_len == customizable_built_in_procedure_cur->name_len) && \
                        (strncmp(customizable_built_in_procedure_cur->name, \
                                 mlua_icg_add_built_in_handlers[i].name, \
                                 customizable_built_in_procedure_cur->name_len) == 0))
                {
                    customizable_built_in_procedure_target = mlua_icg_customizable_built_in_procedure_list_lookup( \
                            customizable_built_in_procedure_list, \
                            mlua_icg_add_built_in_handlers[i].name, \
                            mlua_icg_add_built_in_handlers[i].name_len);
                    if (customizable_built_in_procedure_target == NULL)
                    {
                        if ((ret = mlua_icg_add_built_in_handlers[i].dependence(customizable_built_in_procedure_list)) != 0)
                        { goto fail; }
                    }
                }
            }
        ...
        if (customizable_built_in_procedure_cur->called != 0)
        {
            for (i = 0; i != MLUA_ICG_ADD_BUILT_IN_HANDLERS_COUNT; i++)
            {
                                 ...
                                 customizable_built_in_procedure_cur->name_len) == 0))
                {
                    ...
                    if ((ret = mlua_icg_add_built_in_handlers[i].func(err, \
                                    icode, \
                                    res_id)) != 0)
                    { goto fail; }
